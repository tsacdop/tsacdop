...
class ListenNotesSearch {
  ...
  Future<SearchPodcast<dynamic>?> searchPodcasts({
    ...
    var response = await _dio.get(
      url,
      options: Options(
        headers: {'X-ListenAPI-Key': "$_apiKey", 'Accept': "application/json"},
      ),
    );
    Map searchResultMap = jsonDecode(response.toString());
    var searchResult = SearchPodcast.fromJson(
  ...
  Future<SearchEpisodes<dynamic>?> fetchEpisode({
    ...
    Map searchResultMap = jsonDecode(response.toString());
    var searchResult = SearchEpisodes.fromJson(
      searchResultMap as Map<String, dynamic>,
    );
    return searchResult;
  }
  ...
  Future<SearchTopPodcast<dynamic>?> fetchBestPodcast({
    ...
    Map searchResultMap = jsonDecode(response.toString());
    var searchResult = SearchTopPodcast.fromJson(
      searchResultMap as Map<String, dynamic>,
    );
    return searchResult;
  }
...
class ItunesSearch {
  Future<ItunesSearchResult<dynamic>> searchPodcasts({
    ...
    Map searchResultMap = jsonDecode(response.toString());
    final searchResult = ItunesSearchResult.fromJson(
      searchResultMap as Map<String, dynamic>,
    );
    return searchResult;
  }
