...
import 'package:tsacdop/type/search_api/index_podcast.dart';
                      ...
                      return Container(
                        ...
                        child: TextButton(
                          style: TextButton.styleFrom(
                            disabledForegroundColor: context.accentColor
                                .withValues(alpha: 0.5)
                                .withValues(alpha: 0.38),
                          ),
                          child: Text(context.s.loadMore),
                          onPressed: () => setState(() => _loadItems += 10),
                        ),
                      );
                    }
                    return ListTile(
...
class __ListenNotesSearchState extends State<_ListenNotesSearch> {
  ...
  Future<List<OnlinePodcast>> _getListenNotesList(
    ...
    SearchPodcast<dynamic>? searchResult;
    try {
    ...
    var searchList = <OnlinePodcast>[
      ...?searchResult.results?.cast<OnlinePodcast>(),
    ];
...
class __PodcastIndexSearchState extends State<_PodcastIndexSearch> {
  ...
  Future<List<OnlinePodcast?>> _getPodcatsIndexList(
    String searchText, {
    int? limit,
  }) async {
    if (_limit == 10) _saveHistory(searchText);
    PodcastIndexSearchResult<dynamic> searchResult;
    try {
    ...
    var list = searchResult.feeds?.cast() ?? const [];
...
class _SearchResultDetailState extends State<SearchResultDetail>
  ...
  Future<List<OnlineEpisode>> _getListenNotesEpisodes({
    ...
    final searchEngine = ListenNotesSearch();
    final searchResult = await searchEngine.fetchEpisode(
      id: id,
      nextEpisodeDate: nextEpisodeDate,
    );
    if (searchResult == null) return _episodeList;
    _nextEpisdoeDate = searchResult.nextEpisodeDate;
    _episodeList.addAll(
      searchResult.episodes?.whereType<OnlineEpisode>() ?? const [],
    );
    _loading = false;
    return _episodeList;
  }
  ...
  Future<List<OnlineEpisode>> _getIndexEpisodes({String? id}) async {
    if (id == null || id.isEmpty) return _episodeList;
    final searchEngine = PodcastsIndexSearch();
    final searchResult = await searchEngine.fetchEpisode(rssUrl: id);
    final episodes =
        searchResult.items?.whereType<IndexEpisode>() ?? const <IndexEpisode>[];
    ...
    if (_slideDirection == SlideDirection.up) {
      ...
      if (_move > -50) {
        setState(() {
          _animation = Tween<double>(
            begin: _animation.value,
            end: _animation.value > _minHeight ? widget.maxHeight : _minHeight,
          ).animate(_controller);
          _initSize = _animation.value > _minHeight
              ? widget.maxHeight
              : _minHeight;
        });
        _controller.forward();
        ...
        if (snapshot.hasData) {
          ...
          return ListView.builder(
            ...
            itemBuilder: (context, index) {
              ...
              final episode = content[index];
              final duration = episode.length ?? 0;
              final published = episode.pubDate?.toDate(context) ?? '';
              return ListTile(
                title: Text(episode.title ?? ''),
                tileColor: Colors.transparent,
                subtitle: Text(
                  duration == 0 ? published : '${duration.toTime} | $published',
                  style: TextStyle(color: context.accentColor),
                ),
                trailing: Selector<AudioPlayerNotifier, EpisodeBrief?>(
                  selector: (_, audio) => audio.episode,
                  builder: (_, currentEpisode, _) {
                    if (currentEpisode != null &&
                        currentEpisode.enclosureUrl == episode.audio) {
                      return Padding(
  ...
  Widget build(BuildContext context) {
    final s = context.s;
    final interval = widget.onlinePodcast.interval?.toInterval(context) ?? '';
    final latestPublication =
        widget.onlinePodcast.latestPubDate?.toDate(context) ?? '';
    final metadata = [
      if (interval.isNotEmpty) interval,
      if (latestPublication.isNotEmpty) latestPublication,
    ].join(' | ');
    final imageUrl = widget.onlinePodcast.image;
                        ...
                        tabs: [
                          ...
                          Row(
                            ...
                            children: [
                              Text(s.episode(2)),
                              SizedBox(width: 2),
                              if ((widget.onlinePodcast.count ?? 0) > 0)
                                Container(
                                  padding: const EdgeInsets.only(
                                    left: 5,
                                    right: 5,
            ...
            if (selectedPodcast != null)
              Positioned.fill(
                child: GestureDetector(
                  ...
                  child: Container(
                    color: context.background.withValues(alpha: 0.9),
                  ),
                ),
              ),
            if (selectedPodcast != null)
